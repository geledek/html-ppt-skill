...
def build(course_dir, out_name='index.html', only=None, section_bounds=None):
    ...
    fragments = []
    # Every slide is numbered, including the cover and the generated References
    # slide: a missing number reads as a missing slide (R4-11). Course-slide
    # numbers are kept so they still match course.md and the narration.
    total = max(s[0] for s in SLIDES) + 1      # + References
    ...
    for n, kicker, title, body, refs in SLIDES:
        ...
        # Sources are registered for the References slide but no longer printed
        # on each slide: a marker on every slide became wallpaper (R4-04).
        for r in refs:
        ...
        pos = (f'<span class="course-position" aria-label="Slide {n} of {total}">'
               f'{n:02d} / {total}</span>')
        if is_cover:
            markup = (f'<section class="slide cover" {attrs}>{pos}{body}'
    ...
    if order:
        ...
        fragments.append(
            ...
            f'<span class="course-position" aria-label="Slide {total} of {total}">'
            f'{total:02d} / {total}</span>'
    ...
    # The scale must be set BEFORE first paint. With stage.js at the end of the
    # body the deck paints once at 1920x1080 and then snaps, which reads as a
    # shake on every slide. Transitions are suppressed for the same first frame.
    boot = ('<script>(function(){var d=document.documentElement;'
            'function f(){d.style.setProperty("--stage-scale",'
            'String(Math.min(innerWidth/1920,innerHeight/1080)));}f();'
            'addEventListener("resize",f);d.className+=" booting";'
            'addEventListener("load",function(){requestAnimationFrame(function(){'
            'd.className=d.className.replace(" booting","");});});})();</script>'
            '<style>html.booting *{transition:none!important;animation:none!important}</style>')
    out = ('<!DOCTYPE html>\n<html lang="en"><head><meta charset="utf-8">'
           '<meta name="viewport" content="width=device-width,initial-scale=1">'
           f'<title>{html.escape(title)}</title>' + boot +
           '<!-- GENERATED by scripts/course/engine.py. Edit slides.py and course.md. -->'
