...
def build(course_dir, out_name='index.html', only=None, section_bounds=None):
    ...
    for n, kicker, title, body, refs in SLIDES:
        ...
        head = (f'<header class="slide-head"><p class="kicker">{html.escape(kicker)}</p>'
                f'<h2 class="h2 mt-s">{html.escape(title)}</h2></header>') if kicker or title else ''
        cls = 'slide' + (' cover' if n == 1 else '')
        markup = (f'<section class="{cls}" data-section="{section_for(n, bounds)}" '
                  f'data-course-slide="{n}" data-title="{html.escape(title)}">{head}{pos}'
                  f'<div class="slide-body">{body}<div class="notes">{note_html}</div></div>{src_line}</section>')
