...
The engine reads this file for the narration and generates each slide's hidden
notes from it. Research brief, outline and frontmatter stay in
...
```bash
python3 scripts/course/stamp-times.py courses/ai-value-management
```
...
It estimates a precise zero on earnings and hours, ruling out effects larger than two per cent two years after ChatGPT's launch. That does not mean nothing happened; it means any effect was too small to tell apart from zero.
